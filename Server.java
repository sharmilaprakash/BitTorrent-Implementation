...
//		PrintWriter writer = new PrintWriter(chunk_dir + "/file-info.txt", "UTF-8");
//		writer.println(extname[0] + " " + extname[1]);
//		writer.close();
		...
		listOfFiles = chunk_dir.listFiles(textFilter);
	...
	private static class Handler extends Thread {
		...
		public void run() {
			try {
				...
				for (int i = no - 1; i <= listOfFiles.length - 1; i += client_count) {
				...
				System.out.println("All chunks sent to Client " + no);
